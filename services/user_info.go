...
func UserInfoUpdate(c *fiber.Ctx) error {
	...
	user.Email = utils.IfThenPtr(updateInfo.Email, user.Email)
	user.Name = utils.IfThenPtr(updateInfo.Name, user.Name)
	...
	user.Role = utils.IfThenPtr(updateInfo.Role, user.Role)
